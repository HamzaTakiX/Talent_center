/** Classes de colonnes pour tables admin — largeurs contraintes */

export const ADMIN_TABLE_COL = {
  title: 'admin-table-col admin-table-col--title',
  company: 'admin-table-col admin-table-col--company',
  location: 'admin-table-col admin-table-col--location',
  status: 'admin-table-col admin-table-col--status',
  applicants: 'admin-table-col admin-table-col--applicants',
  deadline: 'admin-table-col admin-table-col--deadline',
  actions: 'admin-table-col admin-table-col--actions',
  image: 'admin-table-col admin-table-col--image',
  name: 'admin-table-col admin-table-col--name',
  email: 'admin-table-col admin-table-col--email',
  text: 'admin-table-col admin-table-col--text',
} as const;
