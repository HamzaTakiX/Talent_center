...
import { profileIntelligenceApi } from '../../api/profileIntelligence';

import type { IntelligencePlatformOverview } from '../../api/types';

import { adminPlatformHealthMock, USE_ADMIN_DASHBOARD_MOCK } from '../data/adminMockData';
...
const PlatformHealthOverview: FunctionComponent = () => {
  ...
  const [data, setData] = useState<IntelligencePlatformOverview | null>(
    USE_ADMIN_DASHBOARD_MOCK ? adminPlatformHealthMock : null,
  );

  useEffect(() => {
    if (USE_ADMIN_DASHBOARD_MOCK) return;

    let cancelled = false;

    void profileIntelligenceApi.overview().then((overview) => {
  ...
  const studentsAtRisk = data?.students_at_risk ?? 0;
  ...
  const formattedUsers = useMemo(() => {
    ...
    return new Intl.NumberFormat(locale).format(activeUsers);

  }, [activeUsers, i18n.language]);
  ...
  return (
    ...
    >
      ...
      <motion.div className="admin-health-overview-main">
        ...
        <HealthRing score={healthScore} label={t('admin.dashboard.health.platformHealth')} />
        ...
        <div className="admin-health-overview-body">
          ...
          <div className="admin-health-trends">
            ...
            >
              ...
              <TrendBars values={riskTrend} variant="risk" />
            ...
            >
              ...
              <TrendBars values={activityTrend} variant="activity" />
