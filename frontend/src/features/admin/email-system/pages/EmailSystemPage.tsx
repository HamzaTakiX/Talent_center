...
import { EmailSystemAlert, EmailSystemTabLoading } from '../ui/EmailSystemPrimitives';
...
const TABS: { id: EmailSystemTab; icon: typeof Mail; labelKey: string }[] = [
  ...
  { id: 'template', icon: Layers, labelKey: 'tabs.template' },
  ...
  return (
    <AdminModulePageShell width="wide">
      ...
      <div className="email-system-tab-stack">
        {activeTab === 'general' ? (
          ws.loading || !generalDraft ? (
            <EmailSystemTabLoading variant="form" />
          ) : (
            <GeneralSettingsTab
              draft={generalDraft}
              saving={ws.saving}
        ...
        {activeTab === 'senders' ? (
          ws.loading ? (
            <EmailSystemTabLoading variant="dual" tableCols={5} />
          ) : (
            <SenderIdentitiesTab
              items={ws.senders}
              saving={ws.saving}
        ...
        {activeTab === 'template' ? (
          ws.loading || !generalDraft ? (
            ...
            <TemplateTab general={generalDraft} />
          )
        ) : null}
