...
import type {
  ...
  DocumentMessage,
  InboxStats,
  PrimaryDocumentFilter,
  PrimaryFilterCounts,
} from '../types/documentChatTypes';
import { EMPTY_DOCUMENT_FILTERS } from '../types/documentChatTypes';
import {
  mapDocumentConversationDto,
  mapDocumentMessages,
} from '../utils/documentChatMappers';
...
function applyPrimaryFilter(conv: DocumentConversation, primary: PrimaryDocumentFilter): boolean {
...
function computePrimaryFilterCounts(conversations: DocumentConversation[]): PrimaryFilterCounts {
  return {
...
export function useDocumentSupportChat() {
  ...
  const loadConversations = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (!options?.silent) {
        setLoadError(err instanceof Error ? err.message : t('admin.common.errors.loadFailed', 'Erreur de chargement'));
  ...
  const conversations = useMemo(() => {
    const mapped = rawConversations.map((dto) => {
      ...
      const conv = mapDocumentConversationDto(
        dto,
        mapDocumentMessages(msgs, studentUserId),
      );
  ...
  const setPrimaryFilter = useCallback((primary: PrimaryDocumentFilter) => {
  ...
  const sendMessage = useCallback(
    async (text: string, files?: File[], tagCodes?: string[], entityRefs?: import('../../../../shared/contextual-chat/types/chatEntityTypes').ChatEntityReference[]) => {
      ...
      try {
        ...
        setMessagesByConv((prev) => {
          const next = {
            ...prev,
            [conversationId]: (prev[conversationId] ?? []).filter((message) => {
              const isTarget =
                isPendingLocalMessage(message) &&
                message.metadata_json?.client_nonce === optimisticId;
    ...
    [selectedId, sendWsTyping, scheduleSilentReload, refreshChatUnread],
  );
