...
import type { PaidStudentDetailRow } from '../data/paidStudentsDetailMock';
import { paidStudentsDetailRows } from '../data/paidStudentsDetailMock';
...
const PaidStudentsDetailTable: FunctionComponent = () => {
  const searchPh = useAdminSearchPlaceholder('students');
  const [query, setQuery] = useState('');
  const [classFilter, setClassFilter] = useState<string>('all');

  const classOptions = useMemo(() => {
    const uniq = [...new Set(paidStudentsDetailRows.map((r) => r.className))].sort();
    return uniq;
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    return paidStudentsDetailRows.filter((r) => {
  ...
  const statusBadge = <span className={srfPaymentTableBadge('Paid')}>Paid</span>;
    ...
    <div className="flex w-full flex-col overflow-hidden admin-module-panel font-inter">
      <div className="box-border flex w-full flex-col gap-6 px-4 pb-6 pt-6 text-left font-inter text-num-14 text-[var(--admin-text-secondary)] sm:px-6">
        ...
        <div className="space-y-3 lg:hidden">
          ...
          rows.map((row: PaidStudentDetailRow) => (
            <AdminMobileRowCard
              key={row.id}
              ...
              fields={[
                ...
                {
                  label: 'Remaining',
                  value: <span className="tabular-nums font-medium text-[#00a63e]">{mad(row.remaining)}</span>
                }
              ]}
              actions={
            ...
            <thead>
              <tr className={`h-10 ${borderRow}`}>
                {headings.map((h) => (
                  <th
                    key={h}
                    className={`align-middle px-2 py-2 pb-2 font-inter text-sm font-medium leading-5 text-[var(--admin-text)] first:pl-0 last:pr-2 ${
            ...
            <tbody>
              ...
              rows.map((row: PaidStudentDetailRow) => (
                <tr key={row.id} className={borderRow}>
                  <td className="h-[49px] align-middle pl-0 pr-4 text-sm font-medium leading-5">
                    {row.studentName}
                  </td>
                  ...
                  <td className="h-[49px] px-2 align-middle text-sm font-medium leading-5 tabular-nums md:px-4">
                    {mad(row.amountPaid)}
                  </td>
                  <td className="h-[49px] px-2 align-middle text-sm font-medium leading-5 tabular-nums text-[#00a63e] md:px-4">
                    {mad(row.remaining)}
                  </td>
...
export default PaidStudentsDetailTable;
