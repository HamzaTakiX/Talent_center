...
import { Search, Eye, CircleCheck } from 'lucide-react';
import { AdminSelectField, AdminSearchEmptyState, AdminTableEmptyState } from '../../../../ui';
import type { PendingValidationRow } from '../data/pendingValidationDetailMock';
import { pendingValidationDetailRows } from '../data/pendingValidationDetailMock';
...
const PendingValidationDetailTable: FunctionComponent = () => {
  const searchPh = useAdminSearchPlaceholder('students');
  const [query, setQuery] = useState('');
  const [classFilter, setClassFilter] = useState<string>('all');

  const classOptions = useMemo(() => {
    const uniq = [...new Set(pendingValidationDetailRows.map((r) => r.className))].sort();
    return uniq;
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    return pendingValidationDetailRows.filter((r) => {
  ...
  const statusBadge = (
    <span className={srfPaymentTableBadge('Pending Validation')}>Pending Validation</span>
  );
    ...
    <div className="flex w-full flex-col overflow-hidden admin-module-panel font-inter">
      <div className="box-border flex w-full flex-col gap-6 px-4 pb-6 pt-6 text-left font-inter text-num-14 text-[var(--admin-text-secondary)] sm:px-6">
        ...
        <div className="space-y-3 lg:hidden">
          ...
          rows.map((row: PendingValidationRow) => (
            <AdminMobileRowCard
              key={row.id}
              ...
              fields={[
                ...
                { label: 'Amount due', value: <span className="tabular-nums font-bold">{mad(row.amountDue)}</span> },
                { label: 'Amount paid', value: <span className="tabular-nums font-bold">{mad(row.amountPaid)}</span> },
                {
                  label: 'Remaining',
                  value: <span className="tabular-nums font-bold text-[#16a34a]">{mad(row.remaining)}</span>
                }
              ]}
              actions={
                <>
        ...
        <div className="hidden w-full min-w-0 overflow-x-auto lg:block">
          <table className="w-full min-w-[940px] border-collapse text-[var(--admin-text)]">
            <thead>
              <tr className={`h-10 ${borderRow}`}>
                {headings.map((h) => (
                  <th
                    key={h}
                    className={`align-middle px-2 py-2 pb-2 font-inter text-sm font-bold leading-5 text-[var(--admin-text)] first:pl-0 last:pr-2 ${
            ...
            <tbody>
              ...
              rows.map((row: PendingValidationRow) => (
                <tr key={row.id} className={borderRow}>
                  ...
                  <td className="h-[49px] px-2 align-middle text-sm font-bold leading-5 tabular-nums text-[#16a34a] md:px-4">
                    {mad(row.remaining)}
                  </td>
...
export default PendingValidationDetailTable;
