...
import type { BlockedStudentDetailRow } from '../data/blockedStudentsDetailMock';
import { blockedStudentsDetailRows } from '../data/blockedStudentsDetailMock';
...
const BlockedStudentsDetailTable: FunctionComponent = () => {
  const searchPh = useAdminSearchPlaceholder('students');
  const [query, setQuery] = useState('');
  const [classFilter, setClassFilter] = useState<string>('all');

  const classOptions = useMemo(() => {
    const uniq = [...new Set(blockedStudentsDetailRows.map((r) => r.className))].sort();
    return uniq;
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    return blockedStudentsDetailRows.filter((r) => {
  ...
  const statusBadge = <span className={srfPaymentTableBadge('Blocked')}>Blocked</span>;
    ...
    <div className="flex w-full flex-col overflow-hidden admin-module-panel font-inter">
      <div className="box-border flex w-full flex-col gap-6 px-4 pb-6 pt-6 text-left font-inter text-num-14 text-[var(--admin-text-secondary)] sm:px-6">
        ...
        <div className="space-y-3 lg:hidden">
          ...
          rows.map((row: BlockedStudentDetailRow) => (
            <AdminMobileRowCard
              key={row.id}
              ...
              fields={[
                ...
                {
                  label: 'Amount due',
                  value: <span className="tabular-nums font-bold">{mad(row.amountDue)}</span>
                },
                {
                  label: 'Amount paid',
...
export default BlockedStudentsDetailTable;
