...
import type { ExemptedStudentDetailRow } from '../data/exemptedStudentsDetailMock';
import { exemptedStudentsDetailRows } from '../data/exemptedStudentsDetailMock';
...
const ExemptedStudentsDetailTable: FunctionComponent = () => {
  const searchPh = useAdminSearchPlaceholder('students');
  const [query, setQuery] = useState('');
  const [classFilter, setClassFilter] = useState<string>('all');

  const classOptions = useMemo(() => {
    const uniq = [...new Set(exemptedStudentsDetailRows.map((r) => r.className))].sort();
    return uniq;
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    return exemptedStudentsDetailRows.filter((r) => {
  ...
  const statusBadge = <span className={srfPaymentTableBadge('Exempted')}>Exempted</span>;
    ...
    <div className="flex w-full flex-col overflow-hidden admin-module-panel font-inter">
      <div className="box-border flex w-full flex-col gap-6 px-4 pb-6 pt-6 text-left font-inter text-num-14 text-[var(--admin-text-secondary)] sm:px-6">
        ...
        <div className="space-y-3 lg:hidden">
          ...
          rows.map((row: ExemptedStudentDetailRow) => (
            <AdminMobileRowCard
              key={row.id}
            ...
            <thead>
              <tr className={`h-10 ${borderRow}`}>
                {headings.map((h) => (
                  <th
                    key={h}
                    className={`align-middle px-2 py-2 pb-2 font-inter text-sm font-bold leading-5 text-[var(--admin-text-secondary)] first:pl-0 last:pr-2 ${
...
export default ExemptedStudentsDetailTable;
