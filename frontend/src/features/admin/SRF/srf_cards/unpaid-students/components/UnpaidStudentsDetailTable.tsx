...
const UnpaidStudentsDetailTable: FunctionComponent = () => {
  ...
  const classOptions = useMemo(() => {
    const uniq = [...new Set(unpaidStudentsDetailRows.map((r) => r.className))].sort();
    return uniq;
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    return unpaidStudentsDetailRows.filter((r) => {
  ...
  const statusBadge = <span className={srfPaymentTableBadge('Unpaid')}>Unpaid</span>;
...
export default UnpaidStudentsDetailTable;
