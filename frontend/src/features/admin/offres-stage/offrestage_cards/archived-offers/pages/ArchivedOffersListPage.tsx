...
import ArchivedOffersListTableContent from '../components/ArchivedOffersListTableContent';
...
const ArchivedOffersListPage: FunctionComponent = () => {
  ...
  const { items: allRows, total, loading, error, refresh } = useStageOffersByStatus('Archived', query);
  ...
  return (
    <AdminListPageShell onBack={() => navigate('/admin/internship-offers')} backTo="offers">
      ...
      <AdminStatDetailPanel
        title={pageTitle('offers.archived.title', { count: totalFormatted })}
        subtitle={filterSubtitle('offers')}
        searchValue={query}
        onSearchChange={setQuery}
        searchPlaceholder={searchPlaceholder}
        toolbarAriaLabel={toolbarAria('filterArchivedOffers')}
        filter1={{
          value: companyFilter,
      ...
      >
        ...
        {loading ? (
          ...
          <ArchivedOffersListTableContent offers={filteredRows} onRefresh={refresh} />
        )}
      </AdminStatDetailPanel>
    </AdminListPageShell>
  );
...
export default ArchivedOffersListPage;
