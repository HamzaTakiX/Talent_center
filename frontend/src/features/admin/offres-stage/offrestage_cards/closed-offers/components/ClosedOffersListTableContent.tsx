...
import type { ClosedOfferListRow } from '../data/closedOffersOnlyMockData';
...
interface ClosedOffersListTableContentProps {
  offers: ClosedOfferListRow[];
}

const ClosedOffersListTableContent: FunctionComponent<ClosedOffersListTableContentProps> = ({
  offers,
}) => {
        ...
        {offers.length === 0 ? (
          ...
          offers.map((offer) => (
            <AdminMobileRowCard
              ...
              title={offer.title}
              badges={
                <span className={offerStatusTableBadge(offer.status)}>{<OfferStatusLabel status={offer.status} />}</span>
              }
              fields={[
        ...
        <AdminTableScroll minWidth="800px" className="admin-table-scroll--panel">
          <thead>
            <tr className="border-b border-[var(--admin-border)]">
              <th className="py-2.5 pl-2 pr-4 text-left text-sm font-semibold leading-5 text-[var(--admin-text-secondary)]">Title</th>
              <th className="py-2.5 px-4 text-left text-sm font-semibold leading-5 text-[var(--admin-text-secondary)]">Company</th>
              <th className="py-2.5 px-4 text-left text-sm font-semibold leading-5 text-[var(--admin-text-secondary)]">Status</th>
              <th className="py-2.5 px-4 text-left text-sm font-semibold leading-5 text-[var(--admin-text-secondary)]">Applicants</th>
              <th className="py-2.5 px-4 text-left text-sm font-semibold leading-5 text-[var(--admin-text-secondary)]">Deadline</th>
              <th className="py-2.5 px-4 text-right text-sm font-semibold leading-5 text-[var(--admin-text-secondary)]">Actions</th>
            </tr>
          </thead>
          <tbody>
            {offers.length === 0 ? (
              ...
              offers.map((offer) => (
                <tr key={offer.id} className="border-b border-[var(--admin-border)] last:border-b-0">
                  ...
                  <td className="py-3 px-4 align-middle text-sm leading-5 text-[var(--admin-text-secondary)]">{offer.company}</td>
...
export default ClosedOffersListTableContent;
