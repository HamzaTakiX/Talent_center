...
const ActiveOffersListPage: FunctionComponent = () => {
  ...
  const { items: allRows, total, loading, error, refresh } = useStageOffersByStatus('Active', query);
  ...
  return (
    <AdminListPageShell onBack={() => navigate('/admin/internship-offers')} backTo="offers">
      ...
      >
        ...
        {loading ? <OffersListLoading /> : <ActiveOffersListTableContent offers={filteredRows} onRefresh={refresh} />}
      </AdminStatDetailPanel>
    </AdminListPageShell>
  );
};
