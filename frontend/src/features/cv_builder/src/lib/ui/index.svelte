...
{:else}
  <div class="quickcv-tab-layout grid gap3 justify-center py4 sm:py10 h-full min-h-0">
    <Tabs class="quickcv-tab-layout__tabs flex h-full min-h-0 flex-col">
      <TabsList class="quickcv-tabs-list shrink-0">
        <Tab value="editor" class="quickcv-tabs-list__tab">
          <i class="i-fa-regular:edit"></i>
          {cvT('cv.forms.mobile.editorTab')}
        </Tab>
        <Tab value="viewer" class="quickcv-tabs-list__tab">
          <i class="i-fluent:eye-28-regular"></i>
          {cvT('cv.forms.mobile.viewerTab')}
        </Tab>
      </TabsList>
