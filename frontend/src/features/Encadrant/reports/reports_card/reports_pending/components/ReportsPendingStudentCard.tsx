...
import {
  ...
  REPORTS_PENDING_FIELD_SUB,
  REPORTS_PENDING_FIELD_SUB_OVERDUE,
...
import {
  REPORTS_PENDING_PROGRESS_FILL,
  REPORTS_PENDING_STATUS_STYLES,
} from '../constants/reportsPendingStyles';
import type { ReportsPendingStudent } from '../types';

interface ReportsPendingStudentCardProps {
  student: ReportsPendingStudent;
  cardClassName?: string;
}

const ReportsPendingStudentCard: FunctionComponent<ReportsPendingStudentCardProps> = ({
  student,
    ...
    >
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h3 className={REPORTS_PENDING_STUDENT_NAME}>{student.name}</h3>
      ...
      <div className="flex min-w-0 flex-col gap-2">
        ...
        <div
          className={REPORTS_PENDING_PROGRESS_TRACK}
          role="progressbar"
        ...
        >
          <div
            className={`h-full rounded-full transition-all ${REPORTS_PENDING_PROGRESS_FILL}`}
...
export default ReportsPendingStudentCard;
