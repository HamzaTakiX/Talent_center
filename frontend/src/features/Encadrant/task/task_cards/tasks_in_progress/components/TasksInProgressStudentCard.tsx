...
import { TASKS_IN_PROGRESS_STUDENT_CARD } from '../constants/tasksInProgressLayout';
import {
  TASKS_IN_PROGRESS_BADGE,
  TASKS_IN_PROGRESS_PROGRESS_FILL,
  TASKS_IN_PROGRESS_PROGRESS_TRACK,
} from '../constants/tasksInProgressStyles';
import type { TasksInProgressStudent } from '../types';

interface TasksInProgressStudentCardProps {
  student: TasksInProgressStudent;
}

const TasksInProgressStudentCard: FunctionComponent<TasksInProgressStudentCardProps> = ({
  student,
}) => {
  const { t } = useTranslation();

  return (
    <article className={TASKS_IN_PROGRESS_STUDENT_CARD}>
      ...
      <div className="flex min-w-0 flex-col gap-2">
        ...
        <div
          className={TASKS_IN_PROGRESS_PROGRESS_TRACK}
          role="progressbar"
        ...
        >
          <div
            className={TASKS_IN_PROGRESS_PROGRESS_FILL}
            style={{ width: `${student.progressPercent}%` }}
          />
        </div>
      </div>

      <span className={TASKS_IN_PROGRESS_BADGE}>{t('encadrant.status.inProgress')}</span>
    </article>
  );
};

export default TasksInProgressStudentCard;
