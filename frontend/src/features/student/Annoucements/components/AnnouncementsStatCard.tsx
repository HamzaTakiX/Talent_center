...
import type { AnnouncementsStatItem } from '../types';
import { announcementsStatColorMap, announcementsStatIconMap } from '../data/announcementsMock';

interface AnnouncementsStatCardProps {
  stat: AnnouncementsStatItem;
}

/** Même structure visuelle que `InternshipOffersStatCard`. */
const AnnouncementsStatCard: FunctionComponent<AnnouncementsStatCardProps> = ({ stat }) => {
  const Icon = announcementsStatIconMap[stat.iconKey];
  const bgColor = announcementsStatColorMap[stat.iconKey];
...
export default AnnouncementsStatCard;
