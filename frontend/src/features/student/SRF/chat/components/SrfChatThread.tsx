...
const SrfChatThread: FunctionComponent = () => {
  const { t, i18n } = useTranslation();
  const toast = useAdminToast();
  const [messages, setMessages] = useState<SrfChatMessage[]>(() => [...studentSrfChatMessages]);
      ...
      <header className="shrink-0 border-b border-solid border-[var(--admin-border)] bg-[var(--admin-bg-elevated)]">
        <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <span className="inline-flex size-10 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-sm font-bold text-white shadow-sm">
              SRF
            </span>
            <div className="min-w-0">
              <h2 className="m-0 truncate text-sm font-bold text-[var(--admin-text)] sm:text-[15px]">
                {t('student.srf.chat.serviceName')}
              </h2>
              <p
                className={`m-0 mt-0.5 flex items-center gap-1.5 text-xs font-medium ${
                  isMuted
              ...
              >
                ...
                {isMuted ? t('admin.chat.muted') : t('student.srf.chat.online')}
              </p>
            </div>
          </div>
