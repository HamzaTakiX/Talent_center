...
const AnnouncementCard: FunctionComponent<{ row: StudentAnnouncementRow }> = ({ row }) => {
  ...
  const base = `student.dashboard.mocks.announcements.${row.id}`;
  ...
  return (
    <article className="student-offer-card">
      <div className="flex w-full min-w-0 items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          ...
          <div className="flex min-w-0 items-center gap-2 text-[13px] text-[var(--admin-text-secondary)]">
            ...
            <span className="truncate">{t(`${base}.company`)}</span>
          </div>
        </div>
        <span
      ...
      <p className="text-[13px] leading-snug text-[var(--admin-text-secondary)]">{t(`${base}.snippet`)}</p>
    </article>
  );
};
