...
const keys = ['applications', 'profileViews', 'messages'] as const;
...
const StudentDashboardAnalytics: FunctionComponent = () => {
  ...
  const legend = useMemo(
    () =>
      legendFromSeries(
        keys.map((key) => ({
          key,
          label: t(`student.dashboard.chart.${key}`),
  ...
  return (
    <DashboardPanel className="admin-section-panel w-full">
      <DashboardSectionHeader
        icon={<BarChart3 strokeWidth={1.75} aria-hidden />}
        title={t('student.dashboard.chart.title')}
        subtitle={t('student.dashboard.chart.subtitle')}
      />

      <motion.div className="space-y-2.5 p-3 sm:space-y-3 sm:p-4">
        <AdminChartLegend items={legend} />
                ...
                <motion.div className="relative" style={{ height: chartHeight }}>
                  ...
                  <motion.div className="absolute inset-x-0 bottom-5 top-0 grid grid-cols-7 items-end gap-0.5 sm:gap-2">
                    {chartLabels.map((dayLabel, index) => (
                      <motion.div key={dayLabel + index} className="flex h-full min-w-0 flex-col items-center justify-end">
