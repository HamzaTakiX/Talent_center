  ...
  return (
    <article className={CV_TOOL_YOUR_CV_CARD}>
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-x-2 gap-y-1">
        <h2 className={CV_TOOL_SECTION_TITLE}>{t('student.internshipOffers.cvTool.yourCv')}</h2>
        <button
          type="button"
          className={CV_TOOL_UPLOAD_LINK}
          onClick={onUploadClick}
          aria-label={t('student.internshipOffers.cvTool.uploadCvAria')}
        >
          <Upload className="h-3.5 w-3.5 shrink-0 sm:h-4 sm:w-4" strokeWidth={2} aria-hidden />
          <span>{t('student.internshipOffers.cvTool.uploadNew')}</span>
        </button>
      </div>
      ...
      <div className={`${CV_TOOL_CV_PREVIEW_CARD} mt-3 max-[429px]:mt-2.5`}>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-px">
          ...
          <p className="m-0 text-[11px] font-normal leading-[14px] text-[#6a7282] sm:text-xs sm:leading-4">
            {t('student.internshipOffers.cvTool.lastUpdatedLabel', { date: cvFile.lastUpdated })}
          </p>
          <p className="m-0 text-[11px] font-normal leading-[14px] text-[#6a7282] sm:text-xs sm:leading-4">
            {t('student.internshipOffers.cvTool.pagesLabel', {
              size: cvFile.sizeLabel,
              count: cvFile.pageCount,
            })}
          </p>
        </div>
