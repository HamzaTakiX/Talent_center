...
class CvIntelligenceOfferInterviewStartView(APIView):
    ...
    def post(self, request, offer_uuid):
        from apps.stage.models import InternshipOffer
        from apps.cv_intelligence.services.offer_ai_coach import start_offer_interview_session
        ...
        raw_count = request.data.get('question_count', 5)
        try:
            question_count = max(3, min(8, int(raw_count)))
        except (TypeError, ValueError):
            question_count = 5

        data = start_offer_interview_session(
            student, offer, lang=lang, question_count=question_count,
        )
        try:
            ...
            activity_tracking_service.track_action(
                ...
                metadata={'offer_uuid': str(offer.uuid)},
            )
    ...
    def post(self, request, offer_uuid):
        ...
        try:
            ...
            activity_tracking_service.track_action(
                ...
                metadata={'offer_uuid': str(offer.uuid), 'score': score},
            )
            schedule_student_recompute(student.pk)
        except Exception:
            pass
