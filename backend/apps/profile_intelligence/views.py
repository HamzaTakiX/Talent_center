...
class CanReadStudentIntelligence(EffectiveHasPermission):
    """Student profile intelligence is part of the students module.

    `@permission_classes` cannot set a class attribute on the view generated by
    `@api_view`, so the required code is applied here — same approach as
    `SrfFinancePermission`. Without it `EffectiveHasPermission` finds no
    `required_permission` and allows every platform admin through.
    """

    def has_permission(self, request, view) -> bool:
        view.required_permission = 'students.manage'
        return super().has_permission(request, view)


@api_view(['GET'])
@permission_classes([IsAuthenticated, IsPlatformAdmin, CanReadStudentIntelligence])
